	...
	void init(void(*preinit)(), void(*init)(), void(*postinit)()) {
		...
		for(int i=0;i<0;i++){
			process.create_thread([]() {
				stdio::print_info("entered thread!");
				auto &framebuffer = framebuffer::framebuffers[0];
				...
				if(!view) {
					stdio::print_error("Error: didn't get a view");
					return;
				}
				...
				for(auto time=0u;;time++){
					const auto padding = 2u;
					for(auto y=0u+padding+time%4;y<view->buffer.height-padding;y+=4)for(auto x=0u+padding;x<view->buffer.width-padding;x++) {
					...
					if(deltaX||deltaY){
						...
						// stdio::print(updateTime, " for ", width, "x", height, " @ ", scale, " \n");
					// }else{
					// 	interrupts::Guard guard;
					// 	graphics2d::update_view(*view);
					}

					// stdio::print("interrupts::_lock_depth: ", raspi::interrupts::_lock_depth.load(), "\n");
		...
		for(auto i=0;i<10;i++)process.create_thread([]() {
			...
			buffer.draw_text(*graphics2d::font::default_sans, "Lots of test text!", 10*scale, (128-20)*scale, 128*scale, 0xffffff);
		...
		// 		stdio::print(" RAM: ", ramWrites, " writes/sec\n");
		// 	}
		// 	vramWrites = 0;
		// 	ramWrites = 0;
		// }
